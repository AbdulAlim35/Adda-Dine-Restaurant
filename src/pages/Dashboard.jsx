...
function Dashboard() {
  ...
  useEffect(() => {
    const activeBloggers = async () => {
      const active = await databaseService.bloggersQuery();
      setBloggers(active);
    };
    activeBloggers();
    promise();
  ...
  const promise = async () => {
    ...
    const push = await databaseService.listQuery(queries);
    setBlogs(push.documents);
  };
  const onDelete = async (data) => {
    await databaseService.dealetePost(data);
  ...
  useEffect(() => {
    const total = async () => {
      const count = await databaseService.blog();
      setToatlBlogs(count);
    };
    total();
  }, []);
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
          <div>
            ...
            <p className="text-gray-600 mt-2">
              Welcome back! Here's what's happening with your menu.
            </p>
          </div>
          <Link
            to="/admin/add-blog"
            className="mt-4 sm:mt-0 inline-flex items-center px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-800 transition-colors duration-200"
          >
           Add New Menu 
          </Link>
        </div>
          ...
          >
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-gray-600">Start writing a new menu</p>
              </div>
            </div>
          </Link>
