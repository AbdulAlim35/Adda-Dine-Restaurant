...
function AllCulinaryIcone() {
   const [blogs, setBlogs] = useState([]);
    const navigate = useNavigate();
    useEffect(() => {
    const fatch = async () => {
      const sent = await culinaryIconeDatabase.listPost();
      setBlogs(sent.documents);
    };
    fatch();
  }, []);
  const onDelete = async (data) => {
    await culinaryIconeDatabase.dealetePost(data);
    toast.success("Successfully Delete");

    let list = await culinaryIconeDatabase.listPost();
    setBlogs(list.documents);
  };
  const onEdit = (e) => {
    let postId = e.$id;
    navigate("/admin/editculinaryicone/" + postId);
  };
  return (
    <>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">All Features</h1>
            </div>
          </div>
...
export default AllCulinaryIcone