...
function AddOfferList() {
    ...
    if (edit) {
      ...
      upload = await offerListDatabase.updatePost(edit, data);
    } else {
      upload = await offerListDatabase.creatPost(data);
  ...
  const loading = async () => {
    const get = await offerListDatabase.getPost(edit);
    setValue("title", get.title);
    setValue("hading", get.hading);
    setValue("date", get.date);
  };
  useEffect(() => {
      if (edit) {
        loading();
