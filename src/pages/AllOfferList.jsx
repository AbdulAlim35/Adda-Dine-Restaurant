...
const [blogs, setBlogs] = useState([]);
    ...
    useEffect(() => {
    const fatch = async () => {
      const sent = await offerListDatabase.listPost();
      setBlogs(sent.documents);
    };
    fatch();
  }, []);
  const onDelete = async (data) => {
    await offerListDatabase.dealetePost(data);
    toast.success("Successfully Delete");

    let list = await offerListDatabase.listPost();
    setBlogs(list.documents);
  };
  const onEdit = (e) => {
    let postId = e.$id;
    navigate("/admin/editofferlist/" + postId);
  };
  return (
    <>
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">All Offer</h1>
        </div>
      </div>
